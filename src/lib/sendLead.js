...
function normalizeUtm(payload) {
  ...
  return {
    utm_source: clean(
      payload.utm_source ?? fromPayload.utm_source ?? fromUrl.utm_source,
      FIELD_LIMITS.utm,
    ),
    utm_medium: clean(
      payload.utm_medium ?? fromPayload.utm_medium ?? fromUrl.utm_medium,
      FIELD_LIMITS.utm,
    ),
    utm_campaign: clean(
    ...
    utm_content: clean(
      payload.utm_content ?? fromPayload.utm_content ?? fromUrl.utm_content,
      FIELD_LIMITS.utm,
    ),
    utm_term: clean(
      payload.utm_term ?? fromPayload.utm_term ?? fromUrl.utm_term,
      FIELD_LIMITS.utm,
    ),
  };
}
...
export async function sendLead(payload) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Lead request failed: HTTP ${response.status}; body=${responseText.slice(0, 300)}`,
      );
    }
